...
const DashboardUser = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const rabResponse = await axios.get(process.env.REACT_APP_API_URL + '/rab_realisasi/'+localStorage.getItem('kegiatan_id'),{
          headers: {
...
const exportToExcel = async () => {
  try {
    const pemasukanResponse = await axios.get(process.env.REACT_APP_API_URL + '/pemasukan/' + localStorage.getItem('kegiatan_id'), {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });
    const pengeluaranResponse = await axios.get(process.env.REACT_APP_API_URL + '/pengeluaran/' + localStorage.getItem('kegiatan_id'), {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
      }
    });
